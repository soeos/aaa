...
([
    ()=>t=>{
        ...
        async function m(){
            try{
                ...
                if([o===t.uk,GM_getValue("updatedAt")===n,Math.min(Date.parse(n)+864e5-Date.now(),0),
        ...
        async function f(){
            const e="function"
            // ==typeof unsafeWindow.locals.get?unsafeWindow.locals.get("uk"):unsafeWindow.locals.uk;
        ...
        function b(){
            ...
            t.on("video:ended",
                ()=>{m().then(t=>{const{expire_time}=t;Math.min(expire_time-Date.now(),0)||n.update({name:"potser",html:'<img style="width: 300px" src="https://pc-index-skin.cdn.bcebos.com/6cb0bccb31e49dc0dba6336167be0a18.png">',tooltip:"",style:{position:"absolute",top:"50px",right:"50px"},click:(t,e)=>{window.open(e.target.src,"_blank")}})})}
            ),
            o.on(e.Events.FRAG_LOADED,d((e,o)=>{m().then(e=>{t.emit("user",e),t.once("user",({expire_time:t})=>{Math.min(t-Date.now(),0)?y():g()})})},1e3*u(420,t.duration/100,t.duration/3)))
        }
        // 修改
    ...
    ()=>t=>{
        const{
            ...
            function c(){
                const{
                    file:r,quality:i,getUrl:c,adToken:u}=o,
                    ...
                    s.update({name:"quality",html:m?l(m.html):"",
                        selector:i.map(t=>({...t})),onSelect:o=>(t.switchQuality(o.url),n.show=`${e.get("Switch Video")}: ${o.html}`,a.set("quality",l(o.html)),l(o.html)),mounted:()=>{const e=a.get("quality");
                            if(e){const o=s.cache.get("quality").option.selector.find(t=>l(t.html)===e);o&&!o.default&&(t.switchQuality(o.url),s.check(o))}}})}
            function u(){t.once("user",({expire_time:e})=>{if(Math.min(e-Date.now(),0)){c();let e=o.id;t.on("restart",()=>{if(e===o.id){const e=t.layers.cache.get("auto-playback");if(e){const{$ref:t}=e;i(t,"display","none")}}else e=o.id,c()})}})}
    ...
    ()=>t=>{
        ...
        function l(){
            t.once("user",({expire_time:t})=>{if(Math.min(t-Date.now(),0)){
    ...
    ()=>t=>{
        ...
        () => t => { 
            ...
            function l() { t.joySound && t.joySound.destroy() } function c() { t.once("user", ({ expire_time: e }) => { if (Math.min(e - Date.now(), 0)) { const e = o.get("joysound"); "boolean" == typeof e && e && r(e), t.on("destroy", l) } else o.del("joysound") }) } 
            return a.add({ html: "声音设置", name: "joysound", tooltip: "", selector: [
                {
                html: "音质增强", name: "high", tooltip: "关闭", switch: !1, onSwitch: t => { const a = !t.switch, { user: s, show: i } = n.user; return s().then(({ expire_time: n }) => { Math.min(n - Date.now(), 0) ? (r(a), t.tooltip = a ? "开启" : "关闭", o.set("joysound", a), e.show = `音质增强: ${t.tooltip}`) : i() }), a }, mounted: (t, e) => { o.get("joysound") && (e.tooltip = "增强", e.switch = !0) } }, 
                { html: "音量增强", name: "volume", tooltip: "0x", range: [0, 0, 5, .1], 
                onRange: t => { const o = t.range[0], { user: a, show: s } = n.user; return a().then(({ expire_time: t }) => { Math.min(t - Date.now(), 0) ? (!function (t) { i().then(e => { e.setVolume(t) }) }(o), e.show = `音量增强: ${Math.round(100 * o)}%`) : s() }), `${Math.round(100 * o) / 100}x` } }] 
        ...
        () => t => { 
            ...
            function i() { t.once("user", ({ expire_time: t }) => { Math.min(t - Date.now(), 0) && r() }) } return a.update({ html: "色彩滤镜", name: "filter", tooltip: "", selector: [{ html: "亮度", name: "brightness", tooltip: 100, range: [100, 0, 255, 1], onRange: t => { const a = t.range[0], { user: s, show: i } = n.user; return s().then(({ expire_time: t }) => { Math.min(t - Date.now(), 0) ? (o.set("filter", { ...o.get("filter"), brightness: a / 100 }), r(), e.show = `亮度: ${a}`) : i() }), a }, mounted: (t, e) => { const { brightness: n = 1 } = { ...o.get("filter") }, a = Math.trunc(100 * n); e.$range.value = a, e.tooltip = a } }, { html: "对比度", name: "contrast", tooltip: 100, range: [100, 0, 255, 1], onRange: t => { const a = t.range[0], { user: s, show: i } = n.user; return s().then(({ expire_time: t }) => { Math.min(t - Date.now(), 0) ? (o.set("filter", { ...o.get("filter"), contrast: a / 100 }), r(), e.show = `对比度: ${a}`) : i() }), a }, mounted: (t, e) => { const { contrast: n = 1 } = { ...o.get("filter") }, a = Math.trunc(100 * n); e.$range.value = a, e.tooltip = a } }, { html: "饱和度", name: "saturate", tooltip: 100, range: [100, 0, 255, 1], onRange: t => { const a = t.range[0], { user: s, show: i } = n.user; return s().then(({ expire_time: t }) => { Math.min(t - Date.now(), 0) ? (o.set("filter", { ...o.get("filter"), saturate: a / 100 }), r(), e.show = `饱和度: ${a}`) : i() }), a }, mounted: (t, e) => { const { saturate: n = 1 } = { ...o.get("filter") }, a = Math.trunc(100 * n); e.$range.value = a, e.tooltip = a } }, { html: "默认", tooltip: "", values: [1, 1, 1] }, { html: "护眼", tooltip: "", values: [.7, .85, .85] }, { html: "柔和", tooltip: "", values: [1.05, .85, .75] }, { html: "清晰", tooltip: "", values: [1.1, 1.05, 1.01] }, { html: "明亮", tooltip: "", values: [1.2, 1, 1.1] }, { html: "高对比", tooltip: "", values: [1, 1.5, 1] }, { html: "黑白", tooltip: "", values: [1, 1.1, 0] }], onSelect: t => { const { user: e, show: s } = n.user; return e().then(({ expire_time: e }) => { if (Math.min(e - Date.now(), 0)) { const e = t.values;["brightness", "contrast", "saturate"].forEach((t, o) => { const n = a.find(t), s = Math.trunc(100 * e[o]); n.tooltip = s, n.$range.value = s }), o.set("filter", { brightness: e[0], contrast: e[1], saturate: e[2] }), r() } else s() }), t.html } }), t.isReady ? i() : t.once("ready", i), { name: "filter" } 
        }, 

        () => t => { 
            const { i18n: e, notice: o, storage: n, plugins: a, setting: s, controls: r, constructor: { utils: { throttle: i } } } = t; 
            function l() { t.once("user", ({ expire_time: e }) => { if (Math.min(e - Date.now(), 0)) { n.get("autoFullscreen") && (t.fullscreenWeb = !0), t.on("video:timeupdate", i(() => { const { start: e, end: o } = { ...n.get("skipTime") }; if (0 == e && 0 == o) return; const { currentTime: a, duration: s } = t, r = [[0, e], [o ? s - o : 0, o ? s : 0]]; for (const [e, o] of r) if (a >= e && a < o) { t.seek = o; break } }, 1e3)), t.on("video:ended", () => { if (n.get("autoNext") && r.cache.get("playlist")) { const t = r.cache.get("playlist").option.selector, e = t[t.findIndex(t => t.default) + 1]; e ? (r.check(e), "function" == typeof e.open && e.open()) : o.show = "没有下一集了" } }) } }) } return s.update({ html: "播放设置", name: "playSetting", icon: "", tooltip: "", selector: [{ html: "自动连播", name: "autoNext", icon: "", tooltip: "关闭", switch: !1, onSwitch: t => { const e = !t.switch, { user: s, show: r } = a.user; return s().then(({ expire_time: a }) => { Math.min(a - Date.now(), 0) ? (t.tooltip = e ? "开启" : "关闭", n.set("autoNext", e), o.show = `自动下一集: ${t.tooltip}`) : r() }), e }, mounted: (t, e) => { n.get("autoNext") && (e.tooltip = "开启", e.switch = !0) } }, { html: "自动全屏", name: "autoFullscreen", icon: "", tooltip: "关闭", switch: !1, onSwitch: e => { const s = !e.switch, { user: r, show: i } = a.user; return r().then(({ expire_time: a }) => { Math.min(a - Date.now(), 0) ? (t.fullscreenWeb = s, n.set("autoFullscreen", s), e.tooltip = s ? "开启" : "关闭", o.show = `自动全屏: ${e.tooltip}`) : i() }), s }, mounted: (t, e) => { n.get("autoFullscreen") && (e.tooltip = "开启", e.switch = !0) } }, { html: "跳过片头", name: "start", tooltip: "0s", range: [0, 0, 120, 1], onChange(t) { const e = t.range[0], { user: s, show: r } = a.user; return s().then(({ expire_time: t }) => { Math.min(t - Date.now(), 0) ? (n.set("skipTime", { ...n.get("skipTime"), start: e }), o.show = `跳过片头: ${e} 秒`) : r() }), e + "s" }, mounted: (t, e) => { const { start: o } = { ...n.get("skipTime") }; o && (e.tooltip = o + "s", e.$range.value = o) } }, { html: "跳过片尾", name: "end", tooltip: "0s", range: [0, 0, 120, 1], onChange(t) { const e = t.range[0], { user: s, show: r } = a.user; return s().then(({ expire_time: t }) => { Math.min(t - Date.now(), 0) ? (n.set("skipTime", { ...n.get("skipTime"), end: e }), o.show = `跳过片尾: ${e} 秒`) : r() }), e + "s" }, mounted: (t, e) => { const { end: o } = { ...n.get("skipTime") }; o && (e.tooltip = o + "s", e.$range.value = o) } }] }), t.isReady ? l() : t.once("ready", l), { name: "playSetting" } }, () => t => { !function (t) { const { proxy: e, storage: o, template: { $player: n, $video: a }, constructor: { FAST_FORWARD_VALUE: s, FAST_FORWARD_TIME: r, utils: { addClass: i, removeClass: l, hasClass: c } } } = t; let u = null, p = !1, d = 1; const h = e => { const { state: a, playbackRate: l = s } = { ...o.get("fastForward") }; a && ("touch" !== e.pointerType || e.isPrimary) && ("mouse" === e.pointerType && 0 !== e.button || t.playing && !t.isLock && (u = setTimeout(() => { p = !0, d = t.playbackRate, t.playbackRate = l, i(n, "art-fast-forward") }, r))) }, m = () => { clearTimeout(u), p && (p = !1, t.playbackRate = d, l(n, "art-fast-forward"), setTimeout(() => t.play())) }, f = t => { "touch" === t.pointerType && m() }; 
            function g() { 
                t.once("user", ({ expire_time: o }) => { 
                    Math.min(o - Date.now(), 0) && 
                    (e(a, "pointerdown", h), 
            ...
            function f() { 
                ...
                () => t => { 
                    ...
                    function n() { 
                        t.once(
                            "user", ({ expire_time: n }) => { 
                                Math.min(n - Date.now(), 0) && (e.hotkey && !o && (t.isFocus || (t.isFocus = !0)), t.on("blur", n => { e.hotkey && !o && (t.isFocus = !0) })) 
                            }) 
                        } 
